...
\subsection{一覧表示}
図\ref{fig:user_show}のように，登録されている都道府県データが表形式で表示される。
\begin{itemize}
...
この画面から、情報の「編集」、「削除」を行う画面への移動、または一覧表示に戻ることができる。
\myfigure{detail}{詳細表示}


\subsection{データ追加}

図\ref{fig:user_new}の新規作成画面にて、必要な情報を入力し「登録」ボタンを押すことで、新しいデータをリストに追加できる。
「登録\&追加」を選択すると、連続してデータを入力することが可能である。
\myfigure{new}{新規作成画面}

「登録」ボタンを押すと，図\ref{fig:user_added}のように追加したデータと共に一覧表示画面に戻る．
...
「更新」ボタンを押すと，図\ref{fig:user_changed}のように編集したデータと共に詳細画面に戻る．
