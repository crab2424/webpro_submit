...
\subsection{データ構造}
サーバー内の配列変数で管理するデータ構造は表\ref{tab:stella_schema}の通りに構成する．

\begin{table}[H]
    \centering
    \caption{星座データ構造}
    \label{tab:stella_schema}
    \begin{tabular}{c|c|c}
        \toprule
        プロパティ名 & データ型 & 説明 \\
        \midrule
...
また，画面のレイアウトおよび配色は，style.cssに定義したスタイルを用いる．cssは，3つのシステムでレイアウトを統一するため，1つのファイルで管理する．
...
本システムにおける各URLとHTTPメソッド，および対応する処理を表\ref{tab:stella_routing}に定義する．

\begin{table}[H]
    \centering
    \caption{ルーティング一覧}
    \label{tab:stella_routing}
    \begin{tabular}{llll}
        \toprule
...
本システムにおける画面間の遷移を図\ref{fig:stella_flowchart}に示す．なお，本システムのページには戻るリンクを配置するため，一覧表示ページ及び詳細表示ページに直接遷移することが可能である．
...
\begin{itemize}
    \item フォーム: GET /stella/create
        \begin{itemize}
            \item 処理: 表\ref{tab:stella_schema}のidを除くすべてのプロパティを入力フィールドとして表示する．
...
\begin{itemize}
    \item URL: /stella/:id
    \item 処理: URLパラメータのidに基づき配列を検索し，対象データを表示する．指定したidに対応するデータが存在しない場合は，一覧表示ページ(/stella)へリダイレクトする．
...
\begin{itemize}
    \item フォーム: GET /stella/edit/:id
        \begin{itemize}
            \item 処理: 対象データを検索し，value属性に現在の値を埋め込んで表示する．指定したidに対応するデータが存在しない場合は，一覧表示ページ(/stella)へリダイレクトする．
...
\begin{itemize}
    \item フォーム: GET /stella/check/:id
        \begin{itemize}
            \item 簡易確認フォームを表示する．指定したidに対応するデータが存在しない場合は，一覧表示ページ(/stella)へリダイレクトする．
            \item 確認後、GET /stella/delete/:idで削除処理を実行する．
        \end{itemize}
    \item 削除処理: GET /stella/delete/:id
        \begin{itemize}
            \item idに対応する配列要素にnullを代入し，データを削除状態にする（配列の要素数は変更しない）．削除対象の指定したidに対応するデータが元から存在しない場合は，一覧表示ページ(/stella)へリダイレクトする．
            \item 削除した要素名をコンソールに出力する．
            \item 削除完了後，一覧画面(/stella)へリダイレクトする．
        \end{itemize}
\end{itemize}
