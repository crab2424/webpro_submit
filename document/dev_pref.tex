...
\begin{table}[H]
    ...
    \begin{tabular}{llll}
        ...
        \midrule
        新規データ作成 & POST & /pref & (処理後一覧へリダイレクト) \\
        データ更新 & POST & /pref/update/:id & (処理後詳細へリダイレクト) \\
...
\subsection{一覧機能 (Index)}
\begin{itemize}
    \item URL: GET /pref
    \item 処理: サーバー変数の全データをEJSに渡し，forEach文を用いてテーブル形式でレンダリングする．
    \item 要素: 「新規登録ボタン」，各行ごとの「詳細リンク」．
\end{itemize}

\subsection{新規作成機能 (New / Create)}
\begin{itemize}
...
\subsection{詳細表示機能 (Show)}
\begin{itemize}
    \item URL: GET /pref/:id
    \item 処理: URLパラメータのIDに基づき配列を検索し，対象データを表示する．
    \item 要素: 「編集ボタン」，「削除ボタン」（formタグによるPOST/DELETE送信），「一覧に戻るボタン」．
\end{itemize}

\subsection{編集・更新機能 (Edit / Update)}
\begin{itemize}
